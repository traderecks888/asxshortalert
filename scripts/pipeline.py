...
def today_awst(): return datetime.now(UTC) + timedelta(hours=AWST_OFFSET_HOURS)
def read_yaml(path): 
...
def _safe_top(df, col, n):
    if df is None or len(df) == 0 or col not in df.columns or n <= 0: return []
    df2 = df.copy()
...
if __name__ == "__main__":
    ...
    pos_high  = _safe_top(pos_sig, "PctShort_pp_num", top_n_pos)
    pos_delta = _safe_top(pos_sig, "Delta_pp_num",    top_n_pos)
